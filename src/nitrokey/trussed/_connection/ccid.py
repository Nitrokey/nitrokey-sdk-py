...
if HAS_CCID_SUPPORT:
    ...
    def _list_ccid_impl(
        *, filter_atr: frozenset[bytes], filter_reader: frozenset[str], exclusive: bool
    ) -> list[CcidConnectionInfo]:
        infos = []

        for r in readers():
            if not any(name in r.name for name in filter_reader):
                continue
            ...
            if atr not in filter_atr:
                continue

            infos.append(CcidConnectionInfo(reader=r.name, atr=atr))
...
def list_ccid(
    *, filter_atr: frozenset[bytes], filter_reader: frozenset[str], exclusive: bool
    ...
    if HAS_CCID_SUPPORT:
        return _list_ccid_impl(
            filter_atr=filter_atr, filter_reader=filter_reader, exclusive=exclusive
        )
    return []
